<script lang="ts">
  ...
  const adjustHeight = () => {
    ...
    const {bottom: popoverBottom, height} = popover.getBoundingClientRect()
...
<div class="relative">
  <div class="absolute top-0 left-0 right-0 z-popover" bind:this={popover} transition:fly|local={{y: 20, duration: 200}}>
    <slot />
  </div>
</div>
