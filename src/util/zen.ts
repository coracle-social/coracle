...
// Configuration
const ZEN_API_CACHE_TTL = 60000 // 1 minute cache
...
export function getRelayUrl(): string {
  ...
  const url = new URL(window.location.href)
  let relayName = url.hostname.replace('ipfs.', 'relay.')
...
export function checkZenReceivability(identities: ProfileIdentities): ZenReceivability {
  ...
  const hasZenCard = !!identities.zencard && 
                     identities.zencard !== 'None' && 
                     identities.zencard.trim() !== ''
...
/**
 * Get the like cost message for display
 */
export function getLikeCostMessage(locale: string = 'en'): string {
  const messages: Record<string, string> = {
    en: `Liking costs ${LIKE_COST} ${ZEN_SYMBOL}EN`,
    fr: `Liker coûte ${LIKE_COST} ${ZEN_SYMBOL}EN`,
  }
  return messages[locale] || messages.en
}

/**
 * Get token type label for display
 * @param tokenType - 'usage' for MULTIPASS or 'property' for ZEN Card
 * @param locale - Language code
 */
export function getTokenTypeLabel(tokenType: 'usage' | 'property', locale: string = 'en'): string {
  const labels: Record<string, Record<string, string>> = {
    en: {
      usage: 'USAGE TOKENS (MULTIPASS)',
      property: 'PROPERTY TOKENS (ZEN Card)',
    },
    fr: {
      usage: 'JETONS D\'USAGE (MULTIPASS)',
      property: 'JETONS DE PROPRIÉTÉ (ZEN Card)',
    },
  }
  return labels[locale]?.[tokenType] || labels.en[tokenType]
}

/**
 * Get token type description for tooltip
 * @param tokenType - 'usage' for MULTIPASS or 'property' for ZEN Card
 * @param locale - Language code
 */
export function getTokenTypeDescription(tokenType: 'usage' | 'property', locale: string = 'en'): string {
  const descriptions: Record<string, Record<string, string>> = {
    en: {
      usage: 'Service credits: likes, relay usage, AI',
      property: 'Cooperative shares (3×⅓ distributed) - Renewal via G1society',
    },
    fr: {
      usage: 'Crédits de service: likes, relais, IA',
      property: 'Parts coopératives (3×⅓ distribués) - Renouvellement via G1society',
    },
  }
  return descriptions[locale]?.[tokenType] || descriptions.en[tokenType]
}
