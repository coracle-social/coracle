...
<div class="flex gap-3">
  ...
  <p class="text-2xl font-bold">{$_("onboarding.allSet")}</p>
</div>
<p>
  {$_("onboarding.askNostr")}
</p>
<p>{$_("onboarding.introduceYourself")}</p>
...
<div class="flex gap-2">
  ...
  <Button class="btn btn-accent flex-grow text-center" {loading} on:click={next}
    >{$_("onboarding.sayHello")}</Button>
</div>
{#if loading}
  <p class="text-center">{$_("onboarding.calculatingPow")}</p>
{:else}
  <Button class="text-center" on:click={skip}>
    {$_("onboarding.skipToFeed")} <i class="fa fa-arrow-right" />
  </Button>
{/if}
