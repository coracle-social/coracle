<script lang="ts">
  ...
  const onScopeChange = scope => {
    if (scope === FilterScope.Custom) {
      _filter = {..._filter, scope, authors: []}
    } else {
      _filter = {..._filter, scope, authors: []}
    }
  }
  ...
  const getFormFilter = () => ({
    ...
    authors: filter.authors?.map(pk => derivePerson(pk).get()),
    scope: filter.authors?.length > 0 ? FilterScope.Custom : filter.scope || FilterScope.Global,
  ...
  $: {
    scopeOptions =
      $follows.size > 0
        ? [FilterScope.Follows, FilterScope.Network, FilterScope.Global, FilterScope.Custom]
        : [FilterScope.Network, FilterScope.Global, FilterScope.Custom]
  }
</script>
    ...
    <Popover
      ...
      <div slot="trigger" class="cursor-pointer">
        <Chip class="mb-2 mr-2 inline-block">
          From {filter.scope || FilterScope.Global}
          <i class="fa fa-caret-down p-1" />
        </Chip>
      </div>
      <div slot="tooltip">
        <Menu>
          <MenuItem on:click={() => modifyFilter({scope: FilterScope.Follows})}>
            <i class="fa fa-user-plus mr-2" /> Follows
          </MenuItem>
          <MenuItem on:click={() => modifyFilter({scope: FilterScope.Network})}>
            <i class="fa fa-share-nodes mr-2" /> Network
          </MenuItem>
          <MenuItem on:click={() => modifyFilter({scope: FilterScope.Global})}>
    ...
    <form on:submit|preventDefault={applyFilter}>
      <Content size="lg">
        ...
        <div class="flex flex-col gap-1">
          ...
          <SelectButton onChange={onScopeChange} value={_filter.scope} options={scopeOptions} />
          {#if _filter.scope === FilterScope.Custom}
